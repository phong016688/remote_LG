 ...
 */
public class DiscoveryManager implements ConnectableDeviceListener, DiscoveryProviderListener, ServiceConfigListener, MediaControl {
...
//        URLServiceSubscription<PlayStateListener> request = new URLServiceSubscription<>((ServiceCommand.ServiceCommandProcessor) this, "playState", null, null);
//        request.addListener(listener);
//        addSubscriptionDiscover(request);
//        return request;
        return null;
    }
    ...
    public void addSubscriptionDiscover() {
...
//       httpServer = new DLNAHttpServer();
//       httpServer.start();
//       Log.d("DLNA", "startDLNA: ");
       //httpServer.getSubscriptions();
       addSubscriptionDiscover();
   }
...
//                        }
                        ...
                        try {
                            HttpConnection connection = HttpConnection.newSubscriptionInstance(
                                   // new URI("http", "", serviceDescription.getIpAddress(), serviceDescription.getPort(), "/hello", "", ""));
                                    new URI("http", "", myIpAddress,port, "/hello", "", ""));
                            connection.setMethod(HttpConnection.Method.SUBSCRIBE);
                            connection.setHeader("CALLBACK", "<http://" + myIpAddress + ":" + port + "/hello" + ">");
                            ...
                            connection.execute();
                            Log.d("HTTP/DLNA", "run: " + connection);
                            if (connection.getResponseCode() == 200) {
                                Log.d("#DLNA", "connection.getResponseCode(): " + connection.getResponseCode());
...
//                }
                    ...
                    case CONNECTED:
                        if (mSearching) {
                            for (DiscoveryProvider provider : discoveryProviders) {
                                Log.d("Connect SDK", "onReceive: "+provider.toString());
                                provider.restart();
                            }
                        }
    ...
    public void addListener(DiscoveryManagerListener listener) {
        Log.d("DiscoveryManagerListener###", "addListener: " + compatibleDevices.values());
    ...
    public void setCapabilityFilters(List<CapabilityFilter> capabilityFilters) {
        ...
        for (ConnectableDevice device: allDevices.values()) {
            Log.d("DiscoveryManagerListener", "setCapabilityFilters: "+allDevices.values());
    ...
    public void registerDeviceService(Class<? extends DeviceService> deviceClass, Class<? extends DiscoveryProvider> discoveryClass) {
        Log.d("registerDeviceService", "registerDeviceService: " + deviceClass.getName());
        ...
        try {
            ...
            Log.d("Connect SDK", "registerDeviceService filter: " + discoveryFilter.getServiceFilter());
    ...
    public void start() {
        ...
        Util.runOnUI(new Runnable() {
            ...
            @Override
            public void run() {
                Log.d("Connect SDK ", "start discoveryProviders: "+discoveryProviders.size());
    ...
    public void handleDeviceAdd(ConnectableDevice device) {
        if (!deviceIsCompatible(device)) 
            return;
        Log.d("DiscoveryManagerListener", "handleDeviceAdd: "+device);
        compatibleDevices.put(getDeviceKey(device), device);
    ...
    public void handleDeviceUpdate(ConnectableDevice device) {
        ...
        if (deviceIsCompatible(device)) {
            Log.d("DiscoveryManagerListener", "handleDeviceUpdate: "+device);
    ...
    public void handleDeviceLoss(ConnectableDevice device) {
        for (DiscoveryManagerListener listenter: discoveryListeners) {
            ...
            Log.d("DiscoveryManagerListener", "handleDeviceLoss: "+device);
        }
    ...
    public Map<String, ConnectableDevice> getAllDevices() {
        Log.d("DiscoveryManagerListener", "handleDeviceAdd: "+allDevices);
        return allDevices;
    }
    ...
    @Override public void onConnectionFailed(ConnectableDevice device, ServiceCommandError error) {
        Log.d(Util.T, "Service onConnectionFailed: " + serviceDescription.getFriendlyName() + " (" + serviceDescription.getServiceID() + ")");

    }
    @Override public void onDeviceDisconnected(ConnectableDevice device) {
        Log.d(Util.T, "Service onDeviceDisconnected: " + serviceDescription.getFriendlyName() + " (" + serviceDescription.getServiceID() + ")");
    ...
    public void onServiceAdded(DiscoveryProvider provider, ServiceDescription serviceDescription) {
        ...
        Log.d("DiscoveryManagerListener", "devKey: " + devKey);
        Log.d("DiscoveryManagerListener", "serviceDescription: " + serviceDescription.getServiceFilter());
...
//        device.setLastSeenOnWifi(currentSSID);
        ...
        if (deviceIsNew){
            Log.d("DiscoveryManagerListener", "deviceIsNew: "+device);
            handleDeviceAdd(device);
        } else{
            Log.d("DiscoveryManagerListener", "deviceIsNew else: "+device);
            handleDeviceUpdate(device);
        }
