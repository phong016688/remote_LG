...
public class WebOSTVKeyboardInput {
    ...
    public void addToQueue(String input) {
        toSend.add(input);
        if (!waiting) {
            Log.d("sendText", "addToQueuet: "+ input);
            sendData();
        }
    }
    ...
    public void sendDel() {
        if (toSend.size() == 0) {
            ...
            Log.d("sendDelete", "sendDel else: ");
            toSend.remove(toSend.size()-1);
        }
    }

    private void sendData() {
        Log.d("`sendText`", "sendData: ");
        waiting = true;
        ...
        ResponseListener<Object> responseListener = new ResponseListener<Object>() {
            ...
            public void onSuccess(Object response) {
                Log.d("sendText", "sendData onSuccess: ");
                waiting = false;
                if (toSend.size() > 0) 
                    sendData();
            ...
            public void onError(ServiceCommandError error) {
                Log.e("sendText", "sendData onError: " + error.getMessage());
                waiting = false;
                if (toSend.size() > 0) 
                    sendData();
            }
