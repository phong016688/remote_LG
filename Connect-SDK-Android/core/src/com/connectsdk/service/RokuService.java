...
public class RokuService extends DeviceService implements Launcher, MediaPlayer, MediaControl, KeyControl, TextInputControl {
    ...
    private void displayMedia(String url, String mimeType, String title,
        ...
        Log.d("####Sr", "displayMedia: " + url);
        String param = "";
        if (mimeType.contains("image")) {
            param = String.format("15985?t=p&u=%s&h=%s&k=%s", HttpMessage.encode(url), HttpMessage.encode(" "),HttpMessage.encode(" "));
        } else if (mimeType.contains("video")) {
            param = String.format(
                    "15985?t=v&h=a&u=%s&k=(null)&videoName=%s&videoFormat=%s",
                    HttpMessage.encode(url),
                    TextUtils.isEmpty(title) ? "(null)" : HttpMessage.encode(title),
                            HttpMessage.encode(mediaFormat));
        } else if (mimeType.contains("audio")) {
            param = String
                    .format("15985?t=a&h=a&u=%s&k=(null)&songname=%s&artistname=%s&songformat=%s&albumarturl=%s",
                            HttpMessage.encode(url),
                            TextUtils.isEmpty(title) ? "(null)" : HttpMessage.encode(title),
    ...
    public void sendText(String input) {
        ...
        ResponseListener<Object> listener = new ResponseListener<Object>() {
            ...
            @Override
            public void onSuccess(Object response) {
                Log.d("sendText", "onSuccess: ");
            }

            @Override
            public void onError(ServiceCommandError error) {
                Log.e("sendText", "onError: "+ error.getMessage());
                Log.e("sendText", "onError: "+ error.getCode());
            }
        };
        ...
        try {
            ...
            e.printStackTrace();
            Log.e("sendText", "Unsupported encoding: " + e.getMessage());
        }

        String uri = requestURL(action, param);

        Log.d("sendText", "RokuService::send() | uri = " + uri);
    ...
    public void sendDelete() {
        ResponseListener<Object> listener = new ResponseListener<Object>() {
            ...
            @Override
            public void onSuccess(Object response) {
                Log.d("sendDelete", "onSuccess: ");
            }

            @Override
            public void onError(ServiceCommandError error) {
                Log.e("sendDelete", "onSuccess: "+ error.getMessage());
            }
        };
...
//         mServiceReachability.start();
        Log.d("Connect SDK", "connect ROku: ");
        connected = true;
